...
async function main() {
  ...
  console.log("图片签名URL:", imageSignedUrl.substring(0, 80) + "...");
...
我愿意去接触新的事物学习新的技术，同时对未来AI的发展充满期望。我坚持，无论是在生活中还是在学习中，我的过往经历都告诉我长时间的坚持要大于短时间的爆发。`;
  ...
  console.log("知识库文本长度:", knowledgeText.length);

  // Step 3: 导入知识库
  console.log("\nStep 3: 导入知识库到 coze_doc_knowledge...");
  ...
  const queries = ["健身", "篮球", "骑行", "爱好", "性格优势"];
  for (const query of queries) {
    ...
    if (searchResponse.code === 0) {
      ...
      searchResponse.chunks.forEach((chunk, i) => {
        ...
        console.log(`  [${i+1}] 得分: ${chunk.score.toFixed(4)}, 含图片: ${hasImage}`);
        console.log(`      预览: ${chunk.content.substring(0, 80)}`);
      });
    } else {
      console.log(`  搜索失败: ${searchResponse.msg}`);
    }
  }
}
